...
exports.createJsonLBasedOnCSV = async (req, res) => {
  try {
    const csvDataSet = []
    fs.createReadStream('./openai-model-data/stackoverflow-data.csv')
      .pipe(csvParser())
      .on('data', (data) => {
              ...
              .then((results) => {
                if (results) {
                  ...
                  if (prompltComplData.length) {
                    // Write a file
                    fs.writeFileSync(
                      './openai-model-data/stackoverflows-data.jsonl',
                      prompltComplData.toString(),
                    )
                  }

                  // return to response.
                  res.json({ data: prompltComplData })
                }
              })
              .catch((error) => {
                console.error(error)
