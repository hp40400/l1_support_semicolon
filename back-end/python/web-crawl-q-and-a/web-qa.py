...
#Prevent rate limit on embedding api
@retry(wait=wait_random_exponential(min=1, max=60), stop=stop_after_attempt(6))
def completion_with_backoff(counter,**kwargs):
    counter[0]+=1
    print(counter)
    #Every 50 request wait 30 seconds -- Change value to rate limit per minute for api
    if counter[0] == 50:
        time.sleep(30)
        counter[0]=0
...
# domain = "docs.novu.co"
# full_url = "https://docs.novu.co/overview/docker-deploy"
...
# counter=[0]
# df['embeddings'] = df.text.apply(lambda x: completion_with_backoff(counter=counter,input=x, engine='text-embedding-ada-002')['data'][0]['embedding'])
...
print(answer_question(df, question="Explain in detail how to configure docker", debug=True))
